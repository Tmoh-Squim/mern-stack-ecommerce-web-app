...
const ProductCard = ({ data,isEvent }) => {
  ...
  return (
    <>
      <div className="w-[50%] h-[auto] mb-[1px] 800px:mb-[2px] bg-white gap-x-[2px] rounded-[5px] p-[2px] shadow-sm 800px:p-3  relative cursor-pointer sm:w-full">
        <div className="flex justify-end"></div>
        <Link to={`${isEvent === true ? `/product/${data._id}?isEvent=true` : `/product/${data._id}`}`}>
          <img
            src={`${data.images && data.images[0]}`}
            alt=""
            className="w-full h-[148px] m-0 rounded-t-lg 800px:rounded-t-none object-cover 800px:object-contain"
          />
        </Link>
